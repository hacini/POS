...
(string)$date22=date('Y-m-d');



if(isset($_POST['date1'])){
    if($_POST['date1']==''){$date1='1800-01-01';} else {(string)$date1=$_POST['date1'];} 
}
if(isset($_POST['date2'])){
    if($_POST['date2']==''){(string)$date2=date("Y-m-d", strtotime("+1 year", strtotime(date('Y-m-d'))));} else {(string)$date2=$_POST['date2'];} 
}

if(isset($_POST['prixMin'])){
    if($_POST['prixMin']==''){$prixMin='0';} else {(string)$prixMin=$_POST['prixMin'];} 
}  

if(isset($_POST['prixMax'])){
    if($_POST['prixMax']==''){$prixMax='100000000000';} else {(string)$prixMax=$_POST['prixMax'];} 
}  
if(isset($_POST['pourcentage'])){
    if($_POST['pourcentage']==''){$pourcentage='100';} else {(string)$pourcentage=$_POST['pourcentage'];} 
}  
...
Where
  prix.PRIXTTC Between $prixMin And $prixMax And
  prix.POURCENTAGEPROMOTION <= $pourcentage And
  prix.PROMOTION = '1' And
  prix.DATEDEBUTPROMOTION >= '$date1' And prix.DATEFINPROMOTION <= '$date2'
Order By
  prix.PRIXTTC Desc";
